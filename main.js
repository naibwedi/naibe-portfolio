...
function mkP() {
  return { x: Math.random() * W, y: Math.random() * H, vx: (Math.random() - .5) * .4, vy: (Math.random() - .5) * .4, r: Math.random() * 1.8 + .4, a: Math.random() };
}
function init() {
  pts = [];
...
document.querySelectorAll('.r').forEach((el, i) => { el.style.transitionDelay = (i % 6) * .07 + 's'; obs.observe(el); });

// COUNTER
const co = new IntersectionObserver(es => {
  es.forEach(e => {
    if (e.isIntersecting) {
      const el = e.target, t = parseInt(el.dataset.target); let c = 0;
